...
//#region Create
// Crea una nuova sala
export const creaSala = async (nomeSala: string): Promise<Sala | null> => {
    const { data, error } = await supabase
      .from('Sala')
      .insert([{ nome_sala: nomeSala }])
      .select();
...
export const creaGiocatore = async (
    idSala: number,
    nome: string,
...
//#region Read
  // Leggi tutte le sale
  export const ottieniTutteLeSale = async (): Promise<Sala[]> => {
    const { data, error } = await supabase
      .from('Sala')
      .select();
  ...
  export const ottieniGiocatoriDiSala = async (idSala: number): Promise<Giocatore[]> => {
    const { data, error } = await supabase
      .from('Giocatore')
      .select()
      .eq('IdSala', idSala);
...
//#region Update
  ...
  export const aggiornaNomeSala = async (idSala: number, nuovoNome: string): Promise<Sala | null> => {
    const { data, error } = await supabase
      .from('Sala')
      .update({ nome_sala: nuovoNome })
      .eq('Id', idSala)
      .select();
  ...
  export const aggiornaGiocatore = async (
    idGiocatore: number,
    nuovoRuolo: string,
...
//#region Delete
  ...
  export const eliminaGiocatore = async (idGiocatore: number): Promise<boolean> => {
  ...
  export const eliminaSala = async (idSala: number): Promise<boolean> => {
...
export const finisciGioco = async (idSala: number): Promise<boolean> => {
    const successo = await eliminaSala(idSala);
    if (successo) {
